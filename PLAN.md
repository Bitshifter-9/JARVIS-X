...
| 14.6 | **Slack scan, and a 4-hour default.** Slack messages were only ever live pushes; now `scan_slack` pulls recent history from every conversation the bot is in — public and private channels, DMs, and **group DMs (mpim)** — and ingests deadlines for whoever is linked, idempotent on the message `ts`. It runs in the connector tick and on **Scan now** (Connections). The connector poll default is now **14400 s (4 hours)** for Gmail and Slack alike (`gmail_poll_seconds`). Each conversation type is listed on its own (`public_channel`, `private_channel`, `mpim`, `im`), so a bot missing one read scope still scans the rest; `scan_slack` returns the `missing_scopes` it hit and the app tells the user which scopes to add and reinstall (manifest carries `groups:read`, `im:read`, `mpim:read`) | ✅ Tests: a group-DM deadline for a linked user is ingested once, an unlinked author is skipped, a re-scan adds nothing, nothing runs without a link, a missing `groups:read` skips only private channels and is reported, and the default is 4 h |
